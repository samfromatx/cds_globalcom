...
$(document).ready(function(){
  ...
  if ($.cookie('vote_id052013')) {
    $("#poll-container").empty();
    votedID = $.cookie('vote_id052013');
...
function formProcess(event){
  ...
  $("#poll-container").fadeOut("slow",function(){
    ...
    $.cookie('vote_id052013', id, {expires: 365});
    });
}
...
function loadResults(data) {
  ...
  var results_html = "<div id='poll-results'><h3>Results:</h3>\n<dl class='graph'>\n";
  for (id in data) {
  ...
  //To show vote count on page - uncomment line below.
  //results_html = results_html+"</dl><p>Total Votes: "+total_votes+"</p></div>\n";
  
  //No total vote count displayed on the page remove if you uncomment out the line above.
  results_html = results_html+"</dl><p></p></div>\n";
