...
$options[1] = 'Assistance in establishing and operating digital/tablet/mobile customer service';
$options[2] = 'Intelligent cross-sells and up-sells';
$options[3] = 'Using the customer touchpoint as an opportunity to better understand your customer base';
$options[4] = 'Designed to provide an optimal customer experience – multiple contact channels, onshored representatives, etc.';
...
// Initialize the DB
require_once('2013/may/flatfile.php');
$db = new Flatfile();
//update the data path to the current month
$db->datadir = '2013/may/data/';
...
//  $ip_result = $db->selectUnique(IP_DB, 0, $_SERVER['REMOTE_ADDR']);
  ...
  if (!isset($_COOKIE['vote_id052013'])/* && empty($ip_result)*/) {
    $row = $db->selectUnique(VOTE_DB, OPT_ID, $id);
    if (!empty($row)) {
//      $ip[0] = $_SERVER['REMOTE_ADDR'];
//      $db->insert(IP_DB, $ip);
      
      setcookie("vote_id052013", $id, time()+31556926); // cookie expires in one year
...
function poll_return_results($id = NULL) {
    ...
    $results_html = "<div id='poll-container'><div id='poll-results'><h3>Mostly, <em>Perspectives</em> is read on a ...</h3>\n<dl class='graph'>\n";
    ...
    $results_html .= "</dl><br class='clear'><p>Total Votes: ". $total_votes ."</p></div></div>\n";
