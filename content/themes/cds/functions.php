...
function cds_add_metaboxes() {
    ...
    add_meta_box('solution_featured', 'Featured Resource', 'cds_featured_resource_metabox', 'page', 'side', 'default');
}
...
function cds_featured_resource_metabox() {
    ...
    // Echo out the field
    echo '<input type="text" name="featured_resource" value="' . $featured_resource  . '" class="widefat" />';
}
function cds_featured_resource_save_meta($post_id, $post) {
    ...
    $events_meta['featured_resource'] = $_POST['featured_resource'];
...
add_action('save_post', 'cds_featured_resource_save_meta', 1, 2); // save the custom fields
